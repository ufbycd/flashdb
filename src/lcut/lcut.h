...
enum
{
	TEST_CASE_SUCCESS = 0, TEST_CASE_FAILURE = 1
};
...
typedef APR_RING_HEAD(lcut_tc_head_t, lcut_tc_t)
lcut_tc_head_t;

typedef struct lcut_ts_t
{
	APR_RING_ENTRY(lcut_ts_t)
	link;
...
typedef APR_RING_HEAD(lcut_ts_head_t, lcut_ts_t)
lcut_ts_head_t;
...
{
	APR_RING_ENTRY(lcut_value_t)
	link;
	void *value;
} lcut_value_t;
typedef APR_RING_HEAD(lcut_value_head_t, lcut_value_t)
lcut_value_head_t;

typedef struct lcut_symbol_t
{
	APR_RING_ENTRY(lcut_symbol_t)
	link;
...
typedef APR_RING_HEAD(lcut_symbol_head_t, lcut_symbol_t)
lcut_symbol_head_t;
